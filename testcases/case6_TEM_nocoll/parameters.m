...
coll_flag = 0.0;% Flag for collisionless simulations if 0
rot_flag  = 0.0;
verbose = 1;
...
Bo = 3.*ones(1,scann); %magnetic field
Ro = 3.*ones(1,scann); %major radius
R0 = 3;
Rmin = 1.*ones(1,scann); %minor radius
x=0.5.*ones(1,scann);	% x is the r/a throughout the scan: impacts the fraction of trapped particles	
rho=x;
qx = 2.00*ones(1,scann); %set q-profile
smag=linspace(-1.1,1.9,scann); %set magnetic shear

alphax=0.*ones(1,scann); %MHD alpha
...
Ati(:,iind)=0.*ones(scann,1); %logarithmic gradient normalized to R
