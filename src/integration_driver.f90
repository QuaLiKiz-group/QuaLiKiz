PROGRAM integration_driver
  ...
  !MPI variables:
  INTEGER :: ierror, nproc, myrank, i,j, write_primi
  ...
  DO j=1,4
     DO i=1,100   
        ...
        SELECT CASE (j)
        CASE (1)
           CALL d01fcf(ndim,aa,bb,minpts,maxpts,rFkstarrstari,relacc2,acc,lenwrk,wrkstr,NAGresult,ifailloc)
           NAGresult=NAGresult*4.; !multiply by 4 due to symmetry           
        CASE (2)
           CALL d01fcf(ndim,aa,bb,minpts,maxpts,iFkstarrstari,relacc2,acc,lenwrk,wrkstr,NAGresult,ifailloc)
           NAGresult=NAGresult*4.; !multiply by 4 due to symmetry           
        CASE (3)
           CALL d01fcf(ndim,aa,bb,minpts,maxpts,rFkstarrstare,relacc2,acc,lenwrk,wrkstr,NAGresult,ifailloc)
           NAGresult=NAGresult*4.; !multiply by 4 due to symmetry
        CASE (4)
           CALL d01fcf(ndim,aa,bb,minpts,maxpts,iFkstarrstare,relacc2,acc,lenwrk,wrkstr,NAGresult,ifailloc)
           NAGresult=NAGresult*4.; !multiply by 4 due to symmetry
!!$        CASE (5)
!!$           NAGresult = d01ahf(cc(1),dd(1),relacc1,npts,relerr,rFFki,lw,ifailloc)
!!$        CASE (6)
!!$           NAGresult = d01ahf(cc(1),dd(1),relacc1,npts,relerr,iFFki,lw,ifailloc)
!!$        CASE (7)
!!$           CALL d01fcf(ndim,cc,dd,minpts,maxpts,rFFke,relacc2,acc,lenwrk,wrkstr,NAGresult,ifailloc)
!!$        CASE (8)
!!$           CALL d01fcf(ndim,cc,dd,minpts,maxpts,iFFke,relacc2,acc,lenwrk,wrkstr,NAGresult,ifailloc)          
        END SELECT
        IF (ifailloc /= 0) THEN
           IF (verbose .EQV. .TRUE.) WRITE(stderr,"(A,I3,A,I7,A,I3,A,G10.3,A,G10.3,A)") 'ifailloc = ',ifailloc,&
                &'. Abnormal termination of integration for j = ',j
     ...
     NAGtime = REAL(time2-time1) / REAL(freq)
     SELECT CASE (j)
     CASE (1)
        WRITE(*,*)
        WRITE(*,'(A,G14.7,A)') 'NAGtime for 100 real passing ion integrations =',NAGtime,' s'
        WRITE(*,'(A,G14.7)') 'NAGresult for rLcirci =', NAGresult
        WRITE(*,*)       
     CASE (2)
        WRITE(*,*)  
        WRITE(*,'(A,G14.7,A)') 'NAGtime for 100 imaginary passing ion integrations =',NAGtime,' s'
        WRITE(*,'(A,G14.7)') 'NAGresult for iLcirci =', NAGresult
        WRITE(*,*)  
     CASE (3)
        WRITE(*,*)  
        WRITE(*,'(A,G14.7,A)') 'NAGtime for 100 real passing electron integrations =',NAGtime,' s'
        WRITE(*,'(A,G14.7)') 'NAGresult for rLcirce =', NAGresult
        WRITE(*,*)  
     CASE (4)
        WRITE(*,*)  
        WRITE(*,'(A,G14.7,A)') 'NAGtime for 100 imaginary passing electron integrations =',NAGtime,' s'
        WRITE(*,'(A,G14.7)') 'NAGresult for iLcirce =', NAGresult
        WRITE(*,*)
!!$     CASE (5)
!!$        WRITE(*,*)  
!!$        WRITE(*,'(A,G14.7,A)') 'NAGtime for 100 real trapped ion integrations (1D) =',NAGtime,' s'
!!$        WRITE(*,'(A,G14.7)') 'NAGresult for rLpiegi =', NAGresult
!!$        WRITE(*,*)  
!!$     CASE (6)
!!$        WRITE(*,*)  
!!$        WRITE(*,'(A,G14.7,A)') 'NAGtime for 100 imaginary trapped ion integrations (1D) =',NAGtime,' s'
!!$        WRITE(*,'(A,G14.7)') 'NAGresult for iLpiegi =', NAGresult
!!$        WRITE(*,*)         
!!$     CASE (7)
!!$        WRITE(*,*)  
!!$        WRITE(*,'(A,G14.7,A)') 'NAGtime for 100 real trapped electron integrations =',NAGtime,' s'
!!$        WRITE(*,'(A,G14.7)') 'NAGresult for rLpiege =', NAGresult
!!$        WRITE(*,*)  
!!$     CASE (8)
!!$        WRITE(*,*)  
!!$        WRITE(*,'(A,G14.7,A)') 'NAGtime for 100 imaginary trapped electron integrations =',NAGtime,' s'
!!$        WRITE(*,'(A,G14.7)') 'NAGresult for iLpiege =', NAGresult
!!$        WRITE(*,*)         
     END SELECT
  ENDDO
  ...
  DO j=1,1
     DO i=1,100   
        ...
        SELECT CASE (j)
        CASE (1)
           CALL coleint(ndim,aa,bb,minpts,maxpts,rFkstarrstari,relacc2,acc,lenwrk,wrkstr,COLEresult,ifailloc)
           COLEresult=COLEresult*4.; !multiply by 4 due to symmetry
        END SELECT
     END DO
     CALL SYSTEM_CLOCK(time2)
     CALL SYSTEM_CLOCK(count_rate=freq)
