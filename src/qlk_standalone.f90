PROGRAM qlk_standalone
  ...
  INTEGER :: int_method, newt_method, newt_conv, int_split
  REAL(KIND=DBL) :: reqrelacc, reqabsacc, reqrelacc_newt, reqabsacc_newt
     ...
     IF (ALLOCATED(oldsol)) THEN !Call with optional old solution input
        ...
        IF (phys_meth == 0) THEN
           CALL qualikiz(dimx, rho, dimn, nions, numsols, phys_meth, coll_flag, rot_flag, verbose,separateflux, kthetarhos, & !general param
                ...
                & int_methodin = int_method, newt_methodin = newt_method, newt_convin = newt_conv, int_splitin = int_split, reqrelaccin = reqrelacc, reqabsaccin = reqabsacc, reqrelacc_newtin = reqrelacc_newt, reqabsacc_newtin = reqabsacc_newt) 
        ENDIF
