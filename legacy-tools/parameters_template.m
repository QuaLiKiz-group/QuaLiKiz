...
xpoints=3;
scann = xpoints * 4;  %Number of points in parameter scan
...
qx=[1*ones(xpoints*2, 1); linspace(1,5,xpoints)'; 1*ones(xpoints, 1)]; %set q-profile
smag=[0.1*ones(xpoints*3, 1); linspace(0.1,5,xpoints)']; %set q-profile
alphax=0.*ones(scann,1); %MHD alpha

%Electrons
Tex=8.*ones(scann,1); % kev
Nex=5.*ones(scann,1); % ne is in units of 10^19
Ate=9.*ones(scann,1); %logarithmic gradient normalized to R
Ane=3.*ones(scann,1); %logarithmic gradient normalized to R
...
Tix(:,iind)=[2.4.*ones(xpoints, 1); linspace(2.4,24,xpoints)'; 2.4.*ones(xpoints*2, 1)]; % keV
ninorm(:,iind)=0.8.*ones(scann,1);  ; % ni/ne arbitrary for main ions (will be rewritten to ensure quasineutrality)
Ati(:,iind)=[linspace(2,12,xpoints)'; 2.*ones(xpoints*3, 1)]; %logarithmic gradient normalized to R
Ani(:,iind) = 3.0.*ones(scann,1); %arbitrary for main ions, will be rewritten to ensure quasineutrality of gradients
...
anis(:,iind)=1.*ones(scann,1); %Tperp/Tpar at LFS. Tixx defined as Tpar
danisdr(:,iind)=0.*ones(scann,1); %d/dr(Tperp/Tpar) at LFS. Tixx defined as Tpar
iind=iind+1;
