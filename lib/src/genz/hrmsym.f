...
C     
C     Reference: Genz, A., and Keister, B. (1996), Fully Symmetric 
C     Interpolatory Rules for Multiple Integrals over Infinite 
C     Regions with Gaussian Weight, J. Comp. Appl. Math., 71, 299-309.
C     
C***  KEYWORDS automatic multidimensional integrator,
C     n-dimensional region (-infin, infin)^n 
C     Gaussian weight
C***  PURPOSE  The routine calculates an approximation to a given
C     vector of definite integrals
C     
C     infin     infin 
C     I    ...  I     w(X)(F ,F ,...,F  ) DX(NDIM)...DX(2)DX(1),
C     -infin    -infin      1  2      NF
C     
C     where F = F (X ,X ,...,X    ), I = 1,2,...,NF,
C     I   I  1  2      NDIM
C     
C     w(X) = EXP(-( X(1)**2 + ... + X(NDIM)**2 )/2)/SQRT(2*PI)**NDIM.
C     
C***  DESCRIPTION Computation of integrals over infinite regions with
C     Gaussian weight.
C     
C     ON ENTRY
C     
C     NDIM   Integer, number of variables, with 1 < NDIM <= 1000.
C     NF     Integer, number of components of the integral.
C     MINPTS Integer, minimum number of function evaluations.
C     MAXPTS Integer, maximum number of function evaluations.
C     FUNSUB Externally declared subroutine for computing
C     all components of the integrand at the given
C     evaluation point.
C     It must have parameters (NDIM,X,NF,FUNVLS)
C     Input parameters:
C     NDIM   Integer that defines the dimension of the
C     integral.
C     X      Real array of dimension NDIM
C     that defines the evaluation point.
C     NF Integer that defines the number of components of I.
C     Output parameter:
C     FUNVLS Real array of dimension NF
C     that defines NF components of the integrand.
C     
...
C     WORK   Real array of working storage, must have dimension at
C     least 2*NF + (NF+1)*NUMSMS. NUMSMS is the number 
C     of NDIM partitions of the integers 0,...,RULE, after
C     RULE+1 approximations to the integrals have been computed.
C     The required value for NUMSMS will never exceed 10000.
C     
C     ON RETURN
C     
C     RESULT Real array of dimension NF.
C     Approximations to all components of the integral.
C     ABSERR Real array of dimension NF.
C     Estimates of absolute accuracies.
C     NEVAL  Integer, number of function evaluations used.
C     IFAIL  Integer.
C     IFAIL = 0 for normal exit, when 
C     ABSERR(K) <= MAX( EPSABS, ABS(RESULT(K))*EPSREL ) for
C     all K, 0 < K <= NF, with <= MAXPTS function values. 
C     IFAIL = 1 if MAXPTS was too small to obtain the required 
C     accuracy. In this case values of RESULT are returned
C     with estimated absolute accuracies ABSERR.
C     WORK   Real array of working storage, that contains information
C     that might be needed for subsequent calls of HRMSYM. This
C     array should not be modified. 
C     
C***  ROUTINES CALLED 
C***  END PROLOGUE HRMSYM
C     
C     Global variables.
C     
      EXTERNAL FUNSUB
...
C     
      ...
      IF ( NDIM .LE. 4 ) THEN 
C     
C     Call product Gauss-Hermite Rule
C     
...
C     APPROXIMATIONS TO A FULLY SYMMETRIC MULTIPLE INTEGRAL. WRITTEN BY 
C     Alan Genz
C     Department of Mathematics
C     Washington State University
C     Pullman, Washington 99164-3113  USA
C     Telephone: 509-335-2131 
C     Electronic Mail: alangenz@wsu.edu
C     Fax: 509-335-1188 
C     
C**************PARAMETERS FOR HRMTRL  ********************************
C*****INPUT PARAMETERS
C     S       INTEGER number of variables, with 0 < S <= 1000.
C     N       Integer, number of components of the integral.
C     F       EXTERNALly declared user defined integrand subroutine.
C     it must have parameters (S,X,N,F), where X is a REAL S-array
C     and F is a real N-array.
...
C     available in the array INTVAL.  For the first call
C     MINORD should be set to -1. Otherwise a previous call is
C     assumed that computed INTVAL. On exit MINORD is set to MAXORD.
C     MAXORD  INTEGER maximum order parameter, must be greater than MINORD
C     and not exceed 25. The subroutine computes approximations of 
C     polynomial degree 2*MAXORD+1.
C******OUTPUT PARAMETERS
C     INTVAL  REAL array of length N.  Upon successful exit
C     INTVAL(1),..., INTVAL(N) are approximations to the components
C     of the integral.  These are all approximations of polynomial 
C     degree 2*MAXORD+1.
C     INTCLS  INTEGER number of F values needed for INTVAL
C     WORK    REAL working storage array. 
C     FULSMS  REAL working storage array with dimension (N+1,*). On exit
C     FULSMS(I,J) contains the fully symmetric basic rule sum
C     indexed by the jth s-partition of the integers 0,...,MAXORD, 
C     for the Ith component of the integrand.
C     FULSMS(N+1,J) contains number of points for the fully 
C     symmetric basic rule sum indexed by the Jth S-partition of 
C     the integers 0,...,MAXORD.
C     NUMSMS  INTEGER number of S-partitions of the integers 0,...,MAXORD.
C***********************************************************************
      EXTERNAL F
...
C     for each D find all distinct partitions M with |M| <= D
C     
...
C     
C     Calculate updated weight for partition M alnd 
C     fully symmetric sums ( when necessary )
C     
...
C     
C     To compute fully symmetric basic rule sums
C     
      INTEGER S, M(*), SUMCLS, IX, LX, I,L, MI, ML, IL, N, MX
      DOUBLE PRECISION X(*), INTWGT, FULSMS(*), FUNVAL(*)
C     
C     Generators for 1 + 2 + 6 + 10 + 16 = 35 point 
C     degree 51 rule, with degree 1, 5, 15 and 29 imbedded rules.     
C     
      PARAMETER ( MX = 17 )
      ...
      END DO
C     
C     Compute centrally symmetric sum for permutation M
C     
 10   DO I = 1, S
         X(I) = -G(M(I))
      END DO
...
C     
C     Subroutine to update weight for partition m
C     
     ...
     &     INTMPA, INTMPB
      SAVE MOMPRD
C     
C     Modified moments and generators for 1 + 2 + 6 + 10 + 16 = 35 point 
C     degree 51 rule, with degree 1, 5 and 19 imbedded rules.     
C     
      PARAMETER ( NZRMAX = 17 )
...
C     
      ...
      END IF
C     
C     Determine Updated Weight Contribution
C     
      DO I = 2,S
         INTRPS(I) = 0
         K(I) = M(I)
...
C
     ...
     &                   INTVAL, FUNS, X, IC )
C
C     Computes product integration rule
C
      EXTERNAL FUNSUB
...
C     Fax: 509-335-1188 
C     
C     KEYWORDS automatic multidimensional integrator, Gaussian weight
C     n-dimensional region (-infin, infin)^n 
C     PURPOSE  The routine calculates an approximation to a vector of
C     definite integrals using Gauss-Hermite product rules.
C     
C     infin     infin
C     I    ...  I       w(X) (F ,F ,...,F      ) DX(NDIM)...DX(2)DX(1),
C     -infin    -infin         1  2      NUMFUN
C     
...
C     
C     DESCRIPTION Computation of integrals over infinite regions with
C     Gaussian weight using Gauss-Hermite product rules.
C     
C     ON ENTRY
C     
C     NDIM   Integer, number of variables.
C     NUMFUN Integer, number of components of the integral.
C     MINPTS Integer, minimum number of function evaluations.
C     MAXPTS Integer, maximum number of function evaluations.
...
C     Input parameters:
C     NDIM   Integer, number of dimensions for the integral.
C     X      Real NDIM-array for the evaluation point.
C     NUMFUN Integer, number of components of I.
C     Output parameter:
C     FUNVLS Real NUMFUN-array for integrand components.
C     
C     EPSABS Real, requested absolute accuracy.
C     EPSREL Real, requested relative accuracy.
C     RESTAR Integer.
...
C     the integral.
C     If RESTAR = 1, then we restart a previous attempt.
C     WORK   Real array of working storage, must have dimensions at
C     least 2*NUMFUN+2*NDIM
C     
C     ON RETURN
C     
C     RESULT Real NUMFUN-array of approximations to all components 
C     of the integral.
C     ABSERR Real NUMFUN-array of estimates of absolute accuracies.
C     NEVAL  Integer, number of function evaluations used.
C     IFAIL  Integer.
C     IFAIL = 0 for normal exit, when 
C     ABSERR(K) <= MAX( EPSABS, ABS(RESULT(K))*EPSREL ) for
C     all K, 0 < K <= NUMFUN, with <= MAXPTS function values. 
C     IFAIL = 1 if MAXPTS was too small to obtain the required 
C     accuracy. In this case values of RESULT are returned
C     with estimated absolute accuracies ABSERR.
C     
C     ROUTINES CALLED MLTRUL
C     END PROLOGUE HERMIT
C     
C     Global variables.
C     
      EXTERNAL FUNSUB
...
C     
 ...
 10   IF ( NEVAL + RULE**NDIM .LE. MAXPTS .AND. RULE .LT. MAXRUL ) THEN
     ...
     &           .OR. RULE .EQ. 1 ) IFAIL = 1
      ...
      END IF
C     
C     Gauss Hermite Weights and Points, N = 1,50
C     
